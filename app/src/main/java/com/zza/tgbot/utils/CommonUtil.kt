 ...
 * 动态数字效果
 */
fun TextView.startNumberDanceAnimation(
    startStr: String,
    startNum: Number,
    endNum: Number,
    duration: Long,
    ...
    val ani = ValueAnimator.ofFloat(startNum.toFloat(), endNum.toFloat()).setDuration(duration)
    val format = "%1\$01.${if (endNum is Float) "2" else "0"}f"
    ani.interpolator = AccelerateInterpolator()
    ani.addUpdateListener {
        text = startStr + String.format(Locale.ENGLISH, format, it.animatedValue)
    }
    ani.addListener(animatorListener)
    ani.start()
}

fun getNowTime(format: String = "YYYY-MM-dd"): String {
    val dateFormat: CharSequence = DateFormat.format(format, System.currentTimeMillis())
 ...
 * 转换为中东地区的数字
 */
val Int.toArNum: String
    get() {
        return NumberFormat.getInstance(Locale("ar")).format(this)
    }


val Float.toArNum: String
    get() {
        return NumberFormat.getInstance(Locale("ar")).format(this)
    }
...
infix fun <T> T?.otherwise(value: T): T = this ?: value
